...
## Phase 5 (2026-09-12 night): install story, Triton paper jobs, OOM-boundary notebook, GIF

- `CQSA_FORWARD=triton` env switch (forward inner) alongside `CQSA_BACKWARD=triton`; the paper experiments were submitted a second time on the Triton kernels (`next/slurm/paper/*_triton.slurm`, jobs 13802006-13802010, outputs `outputs/next/paper_triton/results/`), next to the native set (13795507-13795511, still queued on gpu-short).
- README install rewritten around the zero-build path (Triton kernels, fwd + bwd); the CUDA extension is now the optional 10-15% forward accelerator.
