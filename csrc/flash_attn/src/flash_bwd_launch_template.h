...
void run_flash_bwd_seqk_parallel(Flash_bwd_params &params, cudaStream_t stream) {
    ...
    // Standard FA bwd preprocess computes dsoftmax_sum = row_sum(dO * O).
    // In Stream-CQSA Num/Den bwd mode, dsoftmax_sum is provided by caller
    // as row-wise dDen and must not be overwritten here.
    // cqsa_ext_dpsum: dsoftmax_sum was supplied by the caller (it is a global
    // per-token quantity, so Stream-CQSA computes it once instead of once per
    // subproblem, and O then never has to reach the device at all). The caller
    // is responsible for zeroing dq_accum, which this pass would otherwise do.
    if (!params.cqsa_numden_mode && !params.cqsa_ext_dpsum) {
        if (!params.deterministic) {
