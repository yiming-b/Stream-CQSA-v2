...
__forceinline__ __device__ int cqs_seg_delta(const int* block_base, const int a, const int local_row) {
    if (block_base == nullptr) { return 0; }
    return block_base[local_row / a] + (local_row % a) - local_row;
}
...
template<typename Kernel_traits, bool Is_dropout, bool Is_causal, bool Is_local, bool Has_alibi, bool Is_even_MN, bool Is_even_K, bool Is_softcap, bool Return_softmax, int Cqs_mode, typename Params>
inline __device__ void compute_attn_1rowblock(const Params &params, const int bidb, const int bidh, const int m_block) {
    // next/ v10: Cqs_mode 0 = CQS off at compile time, 1 = on at compile time,
    // 2 = runtime flag. Modes 0/1 fold every CQS branch below; mode 2 keeps
    // them as runtime branches (used for the non-causal CQS-on case, whose
    // compile-time-on instantiation mis-compiles into 2.3x the instructions).
    // next/native: modes 3 (causal, compile-time on) and 4 (non-causal,
    // runtime flag) are the WAVE variants of 1 and 2: several subproblems per
    // launch, with per-subproblem CQS tables (see Flash_fwd_params::cqs_wave).
    const bool Is_cqs = (Cqs_mode == 2 || Cqs_mode == 4) ? params.cqs_enabled : (Cqs_mode == 1 || Cqs_mode == 3);
    // Mode 3: causal wave in the UNIFORM layout (batch of W padded sequences, tables at
    ...
    // next/native: this CTA's CQS tables. In wave mode they are the packed
    // tables offset to this subproblem; otherwise the params fields as before.
    int cqs_tok0 = 0, cqs_blk0 = 0, cqs_bb0 = 0;
    int cqs_nblk = params.cqs_num_blocks;
    if constexpr (Is_wave_uniform) {
        cqs_tok0 = bidb * params.cqs_wave_tok_stride;
        cqs_blk0 = bidb * params.cqs_wave_blk_stride;
        cqs_bb0  = bidb * params.cqs_wave_bb_stride;
    } else if (Is_wave_varlen) {
        cqs_tok0 = params.cu_seqlens_q[bidb];
        cqs_blk0 = params.cqs_blk_cu[bidb];
        cqs_nblk = params.cqs_blk_cu[bidb + 1] - cqs_blk0;
        cqs_bb0  = params.cqs_bb_cu == nullptr ? 0 : params.cqs_bb_cu[bidb];
    }
    const int64_t* cqs_bits_p = params.cqs_group_bits == nullptr ? nullptr : params.cqs_group_bits + cqs_tok0;
    const int64_t* cqs_or_p   = params.cqs_blk_or     == nullptr ? nullptr : params.cqs_blk_or     + cqs_blk0;
    const int64_t* cqs_and_p  = params.cqs_blk_and    == nullptr ? nullptr : params.cqs_blk_and    + cqs_blk0;
    const int* cqs_bb_p = params.cqs_block_base == nullptr ? nullptr : params.cqs_block_base + cqs_bb0;
    const int cqs_seg_align = params.cqs_seg_align;
    ...
    if ((Is_causal || Is_local || !Is_even_MN) && n_block_max <= n_block_min) {
        // next/native: the fp32 copy of the output gets the same zeros.
        if (params.cqs_out_acc_ptr != nullptr) {
            ElementAccum* oacc = reinterpret_cast<ElementAccum *>(params.cqs_out_acc_ptr)
                + binfo.q_offset(params.o_batch_stride, params.o_row_stride, bidb)
                + (index_t)m_block * kBlockM * params.o_row_stride + bidh * params.o_head_stride;
            const int rows = binfo.actual_seqlen_q - m_block * kBlockM;
            for (int i = tidx; i < kBlockM * kHeadDim; i += Kernel_traits::kNThreads) {
                const int r = i / kHeadDim, cidx = i % kHeadDim;
                if (r < rows && cidx < params.d) { oacc[(index_t)r * params.o_row_stride + cidx] = ElementAccum(0); }
            }
        }
        if (params.o_ptr == nullptr) {
            Tensor gLSE0 = get_lse_tile<ElementAccum, Params, kBlockM, Is_even_MN>(params, bidb, bidh, m_block, binfo);
            for (int r = tidx; r < kBlockM; r += Kernel_traits::kNThreads) {
                if (r < binfo.actual_seqlen_q - m_block * kBlockM) { gLSE0(r) = INFINITY; }
            }
            return;
        }
    ...
    if (cqs_bb_p != nullptr) {
        gQ.data() = gQ.data()
                  + cqs_seg_delta(cqs_bb_p, cqs_seg_align, m_block * kBlockM) * params.q_row_stride;
    }
    ...
    auto seg_k = [&](int n) {
        auto t = tKgK(_, _, _, n);
        if (cqs_bb_p != nullptr) {
            t.data() = t.data() + cqs_seg_delta(cqs_bb_p, cqs_seg_align, n * kBlockN) * params.k_row_stride;
        }
        return t;
    };
    auto seg_v = [&](int n) {
        auto t = tVgV(_, _, _, n);
        if (cqs_bb_p != nullptr) {
            t.data() = t.data() + cqs_seg_delta(cqs_bb_p, cqs_seg_align, n * kBlockN) * params.v_row_stride;
        }
        return t;
    };
    // next/native: same accessors with the block-map delta supplied by the caller. The
    // live-tile loop below looks the delta of the NEXT tile up together with its
    // summaries, while the current QK GEMM is in flight, so the dependent global load
    // no longer sits between the barrier and the cp.async issue (measured 5% of the
    // in-place kernel at L=56K).
    auto seg_k_d = [&](int n, int delta) {
        auto t = tKgK(_, _, _, n);
        if (cqs_bb_p != nullptr) { t.data() = t.data() + delta * params.k_row_stride; }
        return t;
    };
    auto seg_v_d = [&](int n, int delta) {
        auto t = tVgV(_, _, _, n);
        if (cqs_bb_p != nullptr) { t.data() = t.data() + delta * params.v_row_stride; }
        return t;
    };
    auto seg_delta_of = [&](int n) { return cqs_bb_p == nullptr ? 0 : cqs_seg_delta(cqs_bb_p, cqs_seg_align, n * kBlockN); };
    ...
    for (int masking_step = 0; masking_step < n_masking_steps; ++masking_step, --n_block) {
        ...
        const int cqs_ts = !Is_cqs ? 0
            : (FLASH_NAMESPACE::cqs_tile_clear_c(cqs_rs, cqs_cs_m, cqs_or_p) ? 0 : 1);
    ...
    if (Is_cqs && n_cur >= n_block_min) {
        cqs_cs_cur = FLASH_NAMESPACE::cqs_col_summary_c(cqs_or_p, cqs_and_p, cqs_nblk,
    ...
    int seg_delta_cur = (n_cur >= n_block_min) ? seg_delta_of(n_cur) : 0;
    for (; n_cur >= n_block_min; ) {
        n_block = n_cur;
        const int cqs_ts = !Is_cqs ? 0
            : (FLASH_NAMESPACE::cqs_tile_clear_c(cqs_rs, cqs_cs_cur, cqs_or_p) ? 0 : 1);

        Tensor acc_s = partition_fragment_C(tiled_mma, Shape<Int<kBlockM>, Int<kBlockN>>{});  // (MMA=4, MMA_M, MMA_N)
        clear(acc_s);
        FLASH_NAMESPACE::cp_async_wait<0>();
        __syncthreads();
        FLASH_NAMESPACE::copy</*Is_even_MN=*/true, Is_even_K>(gmem_tiled_copy_QKV, seg_v_d(n_block, seg_delta_cur), tVsV, tKVcKV, tKVpKV);
        cute::cp_async_fence();
        ...
        if (Is_cqs) {
            while (n_next >= n_block_min) {
                cqs_cs_next = FLASH_NAMESPACE::cqs_col_summary_c(cqs_or_p, cqs_and_p, cqs_nblk,
        ...
        const int seg_delta_next = (n_next >= n_block_min) ? seg_delta_of(n_next) : 0;

        FLASH_NAMESPACE::cp_async_wait<0>();
        __syncthreads();
        if (n_next >= n_block_min) {
            FLASH_NAMESPACE::copy</*Is_even_MN=*/true, Is_even_K>(gmem_tiled_copy_QKV, seg_k_d(n_next, seg_delta_next), tKsK, tKVcKV, tKVpKV);
            // This cp_async_fence needs to be in the if block, otherwise the synchronization
            // isn't right and we get race conditions.
            cute::cp_async_fence();
        }
    ...
    // next/native: wave launches hand over only the fp32 copy and the lse.
    if (params.o_ptr == nullptr) {
        Tensor gLSE = get_lse_tile<ElementAccum, Params, kBlockM, Is_even_MN>(params, bidb, bidh, m_block, binfo);
...
inline __device__ void compute_attn(const Params &params) {
    ...
    FLASH_NAMESPACE::compute_attn_1rowblock<Kernel_traits, Is_dropout, Is_causal, Is_local, Has_alibi, Is_even_MN, Is_even_K, Is_softcap, Return_softmax, Cqs_mode>(params, bidb, bidh, m_block);
}
